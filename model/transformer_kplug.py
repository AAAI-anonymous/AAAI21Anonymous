...
from .output_heads import SentenceClassificationHead, MaskedLMHead, SequenceTaggingHead
...
class TransformerKplugModel(TransformerMASSModel):
    ...
    def forward(self, src_tokens=None, src_lengths=None, prev_output_tokens=None, prev_output_positions=None,
        ...
        #print('kwargs', kwargs)
        """ 
        print('sku_vec_tokens', kwargs['sku_vec_tokens'].shape)
        print('sku_vec_tokens', kwargs['sku_vec_tokens'])
        print('sku_vec_tokens', [self.encoder.dictionary[ii] for ii in kwargs['sku_vec_tokens'][0]])
        print('img_vec_tokens', kwargs['img_vec_tokens'].shape)
        print('img_vec_tokens_len', kwargs['img_vec_tokens_len'].shape)
        print('img_vec_tokens_len', kwargs['img_vec_tokens_len'])
        """
        """
        print('src_tokens', src_tokens.shape)
        print('src_tokens', src_tokens)
        print('src_tokens', ' '.join([self.encoder.dictionary[ii] for ii in src_tokens[0]]))
        print('src_tokens', ' '.join([self.encoder.dictionary[ii] for ii in src_tokens[-1]]))
        """
        attn_sep_mask = src_tokens.clone()
        attn_sep_mask[attn_sep_mask != 102] = 0
        attn_sep_mask[attn_sep_mask == 102] = 1
        """
        print(src_tokens.shape[0], src_tokens.shape[1])
        print('att_sep_mask', attn_sep_mask.shape)
        print('att_sep_mask', attn_sep_mask[0])
        print('src_tokens', src_tokens)
        print(eee)
        """
        sku_vec_tokens = kwargs['sku_vec_tokens']
        img_vec_tokens = kwargs['img_vec_tokens']
        img_vec_tokens_len = kwargs['img_vec_tokens_len']
        ...
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, sku_vec_tokens=sku_vec_tokens, img_vec_tokens=img_vec_tokens, img_vec_tokens_len=img_vec_tokens_len)
        ...
        #print('prev_output_positions', prev_output_positions)
        #print('kplug img_vec_tokens', img_vec_tokens)
        #print('src_tokens', src_tokens.shape)
        ...
        if prev_output_tokens is not None and not prev_output_tokens.eq(self.decoder.padding_idx).all():
            ...
            decoder_out, extra = self.decoder(prev_output_tokens, encoder_out=encoder_out,
                                              prev_output_positions=prev_output_positions, img_vec_tokens=img_vec_tokens, attn_sep_mask=attn_sep_mask)
