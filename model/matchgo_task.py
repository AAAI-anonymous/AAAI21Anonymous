...
from .fairseq_task  import LegacyFairseqTask, FairseqTask
...
def load_langpair_dataset(
    ...
    kb_repeat = data_utils.load_indexed_dataset(kb_repeat_path, tgt_dict, dataset_impl)
    kb_distinct = data_utils.load_indexed_dataset(kb_distinct_path, tgt_dict, dataset_impl)
    
    #print('kb_repeat_path', kb_repeat_path)
    #print('kb_repeat', kb_repeat)

    #print('kb_distinct_path', kb_distinct_path)
    #print('kb_distinct', kb_distinct)
    ...
    if truncate_source:
        ...
        kb_repeat = AppendTokenDataset(
            TruncateDataset(
                StripTokenDataset(kb_repeat, tgt_dict.eos()),
                max_source_positions - 1,
            ),
            tgt_dict.eos(),
        )
        kb_distinct = AppendTokenDataset(
            TruncateDataset(
                StripTokenDataset(kb_distinct, tgt_dict.eos()),
                max_source_positions - 1,
            ),
            tgt_dict.eos(),
        )
    ...
    if append_source_id:
        ...
        kb_repeat = AppendTokenDataset(
            kb_repeat, tgt_dict.index("[{}]".format(kb_repeat_path))
        )
        kb_distinct = AppendTokenDataset(
            kb_distinct, tgt_dict.index("[{}]".format(kb_distinct_path))
        )
        if tgt_dataset is not None:
    ...
    img_vec = IndexedImgDataset(
        img2ids_path=img2ids_path,
        img2vec_path=img2vec_path
    )
...
class MatchgoTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="kb_repeat file path",
        )
        parser.add_argument(
            "--kb-distinct-path",
            type=str,
            default=None,
            help="kb_distinct file path",
        )
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_dataset(
            ...
            kb_repeat_path=self.args.kb_repeat_path + '/' + split + '.kb_val_repeat',
            kb_distinct_path=self.args.kb_distinct_path + '/' + split + '.kb_val_distinct'
        )
...
class IndexedImgDataset(FairseqDataset):
    ...
    def __init__(self, img2ids_path, img2vec_path):
        self.tokens_list = []
        self.sizes = []
        self.lines = []
        self.read_data(img2ids_path, img2vec_path)
        self.size = len(self.tokens_list)
        print('self.size', self.size)

    def read_data(self, img2ids_path, img2vec_path):
        image_feature = np.load(img2vec_path)
        feature_dim = len(image_feature[0])   # 2048
        print('################img2ids_path',img2ids_path)
        print('################img2vec_path', img2vec_path)
